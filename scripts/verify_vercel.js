...
// Tests pure logic only — no network, no database. Run from the repo root: node scripts/verify_vercel.js
...
const {
  ...
  buildM4BonusPrompt
} = require(ROOT + "/api/ai.js")._internal;
...
console.log("\n── timeout budget ──");
const MAX_DURATION_S = JSON.parse(require("fs").readFileSync(ROOT + "/vercel.json", "utf8")).functions["api/ai.js"].maxDuration;
